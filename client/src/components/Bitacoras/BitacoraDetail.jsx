...
const BitacoraDetail = ({edited}) => {
  ...
  useEffect(() => {
    const updateTransportes = async () => {
      if (!bitacora || transportes.length > 0) return; // Prevents updating if transportes is not empty

      if (bitacora.bitacora_id <= 244) {
        const newTransporteData = {
          id: transportes.length + 1,
          tracto: {
        ...
        addTransporte(newTransporteData); // Add to transportes only after setting the state
        console.log("Added new transporteData:", newTransporteData);
      }
    };

    if (initialized) {
      updateTransportes(); // Call the function only once when `initialized` is true
    }
  }, [initialized, bitacora, transportes]);
                ...
                <div className="mb-3">
                  ...
                  <div className="form-check">
                    <input
                      ...
                      checked={selectedTransportes.length === bitacora.transportes.length}
                    />
